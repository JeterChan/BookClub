...
def get_auth_headers(client, email):
    # Helper to get auth headers if not provided by fixture in a way we need
    # Assuming auth_headers fixture is sufficient, but if we need specific user context:
    return {} # Placeholder if auth_headers fixture is used
...
def test_get_notifications_returns_user_notifications(client: TestClient, session: Session, test_user_for_auth: User, auth_headers: dict):
    """測試獲取通知列表返回當前用戶的通知"""
    # 創建另一個用戶
    user2 = User(email="user2@test.com", password_hash="hash", display_name="User 2")
    session.add(user2)
    session.commit()
    ...
    notif1 = Notification(
        ...
        type=NotificationType.CLUB_JOIN_REQUEST, # Using a valid type
        title="Topic 1",
    ...
    # 獲取通知
    response = client.get(f"{settings.API_V1_STR}/notifications", headers=auth_headers)
    ...
    assert len(data) == 2
    assert all(n['user_id'] == test_user_for_auth.id for n in data)
...
def test_get_notifications_filters_by_read_status(client: TestClient, session: Session, test_user_for_auth: User, auth_headers: dict):
    ...
    # 只獲取未讀通知
    response = client.get(f"{settings.API_V1_STR}/notifications?is_read=false", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    ...
    # 只獲取已讀通知
    response = client.get(f"{settings.API_V1_STR}/notifications?is_read=true", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_mark_notification_as_read_success(client: TestClient, session: Session, test_user_for_auth: User, auth_headers: dict):
    ...
    # 標記為已讀
    response = client.put(f"{settings.API_V1_STR}/notifications/{notif.id}/read", headers=auth_headers)
    assert response.status_code == 200
