...
def test_get_all_tags(client: TestClient, session: Session):
    ...
    response = client.get("/api/v1/interest-tags")
...
def test_create_custom_tag_api(authenticated_client: TestClient, auth_headers: dict):
    """測試創建自定義標籤的 API"""
    tag_name = "My API Tag"
    response = authenticated_client.post("/api/v1/interest-tags", headers=auth_headers, json={"name": tag_name})
...
def test_add_and_remove_user_interest_tag_api(authenticated_client: TestClient, auth_headers: dict):
    ...
    tag_response = authenticated_client.post("/api/v1/interest-tags", headers=auth_headers, json={"name": "Tag for User"})
    ...
    response_profile = authenticated_client.get("/api/v1/users/me/profile", headers=auth_headers)
    ...
    response_delete = authenticated_client.delete(f"/api/v1/users/me/interest-tags/{tag_id}", headers=auth_headers)
    assert response_delete.status_code == 204

    # 5. Verify the tag is no longer in the user's profile
    response_profile_after = authenticated_client.get("/api/v1/users/me/profile", headers=auth_headers)
...
def test_create_tag_unauthenticated_fails(client: TestClient):
    """測試未認證用戶無法創建標籤"""
    response = client.post("/api/v1/interest-tags", json={"name": "Unauthorized Tag"})
