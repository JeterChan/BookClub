...
/**
 * Pagination - 分頁元件
 * 顯示當前頁碼、總頁數、總項目數，以及上一頁/下一頁按鈕
 */
export const Pagination = ({ pagination, onPageChange }: PaginationProps) => {
  const { page, total_pages, total_items, has_next, has_previous } = pagination;
      ...
      <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        <div>
          <p className="text-sm text-gray-700">
            顯示第 <span className="font-medium">{(page - 1) * pagination.page_size + 1}</span> 到{' '}
            <span className="font-medium">
              {Math.min(page * pagination.page_size, total_items)}
            </span>{' '}
            項，共 <span className="font-medium">{total_items}</span> 個讀書會
          </p>
        </div>
        <div>
          <nav className="inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
            ...
            <span className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-900 bg-white border border-gray-300">
              {page} / {total_pages}
            </span>
            <button
              onClick={() => onPageChange(page + 1)}
